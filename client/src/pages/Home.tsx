...
const gallery = [
  ...
  { src: "https://images.unsplash.com/photo-1511285560929-80b456fea0bc?auto=format&fit=crop&w=1200&q=85", alt: "Detail bunga putih dalam suasana pernikahan", caption: "Hal-hal kecil" },
...
export default function Home() {
  ...
  return <div className={`site ${opened ? "is-open" : ""}`}>
    ...
    <main id="top">
      ...
      <section id="galeri" className="gallery-section section-shell"><span className="archive-stamp">CONTACT SHEET · 04</span><div className="section-index">03 / GALERI</div><div className="gallery-head reveal"><p className="eyebrow">A few frames from our story</p><h3>Potongan<br /><i>hari-hari kami.</i></h3></div><div className="gallery-grid">{gallery.map((item, i) => <button key={item.src} className={`gallery-item item-${i + 1} reveal`} onClick={() => setLightbox(i)} aria-label={`Lihat foto: ${item.alt}`}><img src={item.src} alt={item.alt} /><span>Lihat foto <ArrowRight size={14} /></span></button>)}</div></section>
